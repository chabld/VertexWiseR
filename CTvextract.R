...
#' @param template: A string object containing the name of surface template (currently available: 'fsaverage5', 'fsaverage6')
#'
...
CTvextract=function(sdirpath, template='fsaverage5') 
{ 
...
system(paste0("ln -s $FREESURFER_HOME/subjects/", template, " -t $SUBJECTS_DIR \n
       mris_preproc --f ./sublist.txt --target fsaverage5 --hemi lh --meas thickness --out lh.mgh \n 
       mris_preproc --f ./sublist.txt --target fsaverage5 --hemi rh --meas thickness --out rh.mgh"))
